...
# -------------------------
# Serve static files (bot-tracker.js for third-party embedding)
# Accessible at: GET /static/bot-tracker.js
# -------------------------
...
# -------------------------
# Model — hot-reloadable (auto-detects when model_v2.pkl is updated on disk)
# -------------------------
...
def get_model():
    ...
    try:
        ...
        if _model is None or mtime > _model_mtime:
            ...
            _model_mtime = mtime
            print(f"\u2705 Model (re)loaded from {MODEL_PATH}")
    except Exception as e:
        print(f"\u26a0\ufe0f  Model load failed: {e}")
    return _model

get_model()  # Eager load at startup
...
def collect_data(data: SessionData):
    try:
        ...
        model = get_model()
        if model is None:
            return {"status": "error", "message": "Model not loaded — check model_v2.pkl"}
...
def reload_model():
    ...
    m = get_model()
    if m is None:
        return {"status": "error", "message": "model_v2.pkl not found"}
    return {"status": "ok", "message": f"Model reloaded from {MODEL_PATH}"}


# -------------------------
# Debug — latest prediction
# -------------------------
@app.get("/debug/latest")
def debug_latest():
    doc = predictions_collection.find_one(sort=[("updated_at", -1)])
    if not doc:
